...
class GridHeightTest extends WebTestCase
{
	...
	public function testEmptyRequest()
	{
		...
		$this->assertEquals([Game::WIDTH => Game::ERROR_HEIGHT_WIDTH_INVALID], json_decode($this->client->getResponse()->getContent(), true));
	}
	...
	public function testHeightNotSet()
	{
		...
		$this->client->request('POST', $uri, [], [], ['CONTENT_TYPE' => 'application/json'], json_encode($data));
		$this->assertEquals(Response::HTTP_BAD_REQUEST, $this->client->getResponse()->getStatusCode());
		$this->assertEquals([Game::HEIGHT => Game::ERROR_HEIGHT_WIDTH_INVALID], json_decode($this->client->getResponse()->getContent(), true));
	}
	...
	public function testHeightInvalidType()
	{
		...
		$this->client->request('POST', $uri, [], [], ['CONTENT_TYPE' => 'application/json'], json_encode($data));
		$this->assertEquals(Response::HTTP_BAD_REQUEST, $this->client->getResponse()->getStatusCode());
		$this->assertEquals([Game::HEIGHT_WIDTH => Game::ERROR_HEIGHT_WIDTH_INVALID], json_decode($this->client->getResponse()->getContent(), true));
	}
